...
// static function
bool SimpleHapticsController::readInPortData(const double& dt, SimpleHapticsController::Ports& ports, cnoid::BodyPtr refRobotRaw, cnoid::BodyPtr actRobotRaw, std::vector<cnoid::Vector6>& refEEWrenchOrigin, std::vector<cpp_filters::TwoPointInterpolatorSE3>& refEEPoseRaw){
...
bool SimpleHapticsController::writeOutPortData(SimpleHapticsController::Ports& ports, const SimpleHapticsController::ControlMode& mode, cpp_filters::TwoPointInterpolator<double>& idleToAbcTransitionInterpolator, double dt, const GaitParam& gaitParam){
...

  {
    ...
    if(mode.now() == SimpleHapticsController::ControlMode::MODE_IDLE){
...
RTC::ReturnCode_t SimpleHapticsController::onExecute(RTC::UniqueId ec_id){
  ...
  if(!SimpleHapticsController::readInPortData(this->dt_, this->ports_, this->gaitParam_.refRobotRaw, this->gaitParam_.actRobotRaw, this->gaitParam_.refEEWrenchOrigin, this->gaitParam_.refEEPoseRaw)) return RTC::RTC_OK;  // qRef が届かなければ何もしない
  ...
  if(this->mode_.isABCRunning()) {
    ...
    SimpleHapticsController::execSimpleHapticsController(this->mode_, this->gaitParam_, this->dt_, this->footStepGenerator_, this->legCoordsGenerator_, this->refToGenFrameConverter_, this->actToGenFrameConverter_, this->impedanceController_, this->stabilizer_,this->externalForceHandler_, this->fullbodyIKSolver_, this->legManualController_, this->cmdVelGenerator_);
  }
...
bool SimpleHapticsController::goPos(const double& x, const double& y, const double& th){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool SimpleHapticsController::goVelocity(const double& vx, const double& vy, const double& vth){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool SimpleHapticsController::goStop(){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool SimpleHapticsController::jumpTo(const double& x, const double& y, const double& z, const double& ts, const double& tf){
  std::lock_guard<std::mutex> guard(this->mutex_);
  return true;
}

bool SimpleHapticsController::setFootSteps(const OpenHRP::SimpleHapticsControllerService::FootstepSequence& fs){
  OpenHRP::SimpleHapticsControllerService::StepParamSequence sps;
...
bool SimpleHapticsController::setFootStepsWithParam(const OpenHRP::SimpleHapticsControllerService::FootstepSequence& fs, const OpenHRP::SimpleHapticsControllerService::StepParamSequence& sps){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
void SimpleHapticsController::waitFootSteps(){
  while (this->mode_.isABCRunning() && !this->gaitParam_.isStatic()) usleep(1000);
  usleep(1000);
  return;
}

bool SimpleHapticsController::releaseEmergencyStop(){
  std::lock_guard<std::mutex> guard(this->mutex_);
  return true;
}


bool SimpleHapticsController::startAutoBalancer(){
...
bool SimpleHapticsController::startStabilizer(void){
...
bool SimpleHapticsController::stopStabilizer(void){
...
bool SimpleHapticsController::startImpedanceController(const std::string& i_name){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
    for(int i=0;i<this->gaitParam_.eeName.size();i++){
...
bool SimpleHapticsController::stopImpedanceController(const std::string& i_name){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
    for(int i=0;i<this->gaitParam_.eeName.size();i++){
      if(this->gaitParam_.eeName[i] != i_name) continue;
...
bool SimpleHapticsController::startWholeBodyMasterSlave(void){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool SimpleHapticsController::stopWholeBodyMasterSlave(void){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool SimpleHapticsController::getFootStepState(OpenHRP::SimpleHapticsControllerService::FootStepState& i_param) {
  std::lock_guard<std::mutex> guard(this->mutex_);
  ...
  for(int i=0;i<NUM_LEGS;i++){
    ...
    SimpleHapticsController::copyEigenCoords2FootStep(this->gaitParam_.genCoords[i].value(), i_param.leg_coords[i]);
    ...
    SimpleHapticsController::copyEigenCoords2FootStep(this->gaitParam_.srcCoords[i], i_param.leg_src_coords[i]);
    i_param.leg_dst_coords[i].leg = this->gaitParam_.eeName[i].c_str();
    SimpleHapticsController::copyEigenCoords2FootStep(this->gaitParam_.footstepNodesList[0].dstCoords[i], i_param.leg_dst_coords[i]);
  }
  ...
  SimpleHapticsController::copyEigenCoords2FootStep(mathutil::orientCoordToAxis(mathutil::calcMidCoords(this->gaitParam_.footstepNodesList[0].dstCoords, weights), cnoid::Vector3::UnitZ()), i_param.dst_foot_midcoords);
...
// static function
void SimpleHapticsController::copyEigenCoords2FootStep(const cnoid::Position& in_fs, OpenHRP::SimpleHapticsControllerService::Footstep& out_fs){
  out_fs.pos.length(3);
